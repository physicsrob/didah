...
describe('handleLiveCopyCharacter - integration', () => {
  ...
  it('completes emission and publishes snapshot', async () => {
    ...
    const handlerPromise = handleLiveCopyCharacter(config, 'A', startTime, ctx, signal.signal, null, false);
  ...
  it('updates remaining time after emission', async () => {
    ...
    const handlerPromise = handleLiveCopyCharacter(config, 'B', startTime, ctx, signal.signal, null, false);
  ...
  it('does not call updateStats (Live Copy mode has no stats during session)', async () => {
    ...
    const handlerPromise = handleLiveCopyCharacter(config, 'C', clock.now(), ctx, signal.signal, null, false);
  ...
  it('handles multiple characters in sequence', async () => {
    ...
    for (const char of chars) {
      const handlerPromise = handleLiveCopyCharacter(config, char, startTime, ctx, signal.signal, null, false);
  ...
  it('uses Farnsworth spacing when configured', async () => {
    ...
    const handlerPromise = handleLiveCopyCharacter(config, 'G', startTime, ctx, signal.signal, null, false);
  ...
  it('logs emission events for evaluation at session end', async () => {
    ...
    const handlerPromise = handleLiveCopyCharacter(config, 'H', startTime, ctx, signal.signal, null, false);
