...
### Star Rating
Based on accuracy across all **50 practice characters**:
- **3 stars**: ≥ 95% accuracy
- **2 stars**: ≥ 90% accuracy
- **1 star**: ≥ 85% accuracy
- **0 stars**: < 85% accuracy
...
- **Practice characters**: Generated by backend source endpoint (`/api/sources/koch-level-{N}`)
  ...
  - Generates 50 weighted characters based on mastery (un-mastered weight=2, mastered weight=1)
...
### State Management
- Track characters completed: count of 50
...
[Load historical stats to determine un-mastered chars]
         ↓
[Backend generates 50 weighted characters]
         ↓
    [Start Session]
         ↓
  ┌──────────────────────────┐
...
**Backend Generation** (50 chars):
- Weight 2: R, A, P, T (un-mastered)
- Weight 1: K, M, S, U (mastered)
- Selection pool: [R, R, A, A, P, P, T, T, K, M, S, U] → random selection → 50 chars

**Example Sequence**: K, R, M, R, A, T, P, K, R, S, A, U, P, T, R, ... (50 total)
...
**Total Session**: 50 characters
**Counted in Statistics**: All 50 characters (first encounters logged as correct, quiz responses logged as answered)
